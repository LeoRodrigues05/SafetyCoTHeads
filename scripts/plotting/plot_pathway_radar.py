...
Usage:
    .venv/bin/python -m scripts.plotting.plot_pathway_radar            # olmo3_7b_think
    .venv/bin/python -m scripts.plotting.plot_pathway_radar --model qwen3_8b
"""
...
JUDGE_ROOT = ROOT / "runs" / "direction_a_v6" / "judge"
...
# steering_a1.5 is deliberately NOT the steering representative: on
# olmo3_7b_think it retains no coherence-gate-passing completion and emits no
# closed </think>, so it has no profile to plot. a1.0 is the strongest dose that
# does.
OVERLAY_REPS = [
    ("steering_a1.0", "Steering a1.0", "#1f77b4"),
...
def plot_radars(model: str, agg: dict, plots_dir: Path, rmax: float) -> Path:
    ...
    for ax, fam in zip(axes.flat, fams):
        ...
        ax.set_theta_direction(-1)
        ax.set_xticks(ang)
        ax.set_xticklabels([SHORT[l] for l in LABELS], fontsize=10)
        ax.set_ylim(0, rmax)
        ax.set_rlabel_position(0)
        ax.tick_params(axis="y", labelsize=8, colors="0.5")
        ax.grid(alpha=.4)
    ...
    stem = plots_dir / f"pathway_radar_{model}"
    for path in save_fig(fig, stem):
        print(f"  wrote {path.relative_to(ROOT)}")
...
def plot_overlay_radar(model: str, agg: dict, plots_dir: Path, rmax: float) -> Path:
    ...
    ax.set_theta_direction(-1)
    ax.set_xticks(ang)
    ax.set_xticklabels([SHORT[l] for l in LABELS], fontsize=11)
    ax.set_ylim(0, rmax)
    ax.set_rlabel_position(0)
    ax.tick_params(axis="y", labelsize=8, colors="0.5")
    ax.grid(alpha=.4)
    ...
    fig.suptitle(f"Pathway-label signature — {model}\n"
                 ...
                 "dataset mean)", fontweight="bold", fontsize=13, y=1.02)
    fig.tight_layout()
    stem = plots_dir / f"pathway_radar_overlay_{model}"
    for path in save_fig(fig, stem):
        print(f"  wrote {path.relative_to(ROOT)}")
    return stem.with_suffix(".png")
...
def main() -> int:
    ...
    if not agg:
        print(f"[error] no pathway rows under {JUDGE_ROOT / args.model}")
        return 1
