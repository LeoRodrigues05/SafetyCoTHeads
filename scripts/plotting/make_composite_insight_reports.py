...
changes:
  ...
  figures/paper/composite_01_steering_dose_response.png
  figures/paper/composite_09_family_dose_response.png
  figures/paper/composite_10_dose_response_compact.png
  figures/paper/composite_02_family_mean_sfs.png
  figures/paper/composite_03_raw_asr_vs_sfs.png
  figures/paper/composite_04_model_family_sfs_heatmap.png
  figures/paper/composite_07_pareto_frontier_clean.png
  figures/paper/composite_08_metric_diagnostics.png
...
Usage:
    .venv/bin/python -m scripts.plotting.make_composite_insight_reports
"""
...
CSV_DEFAULT = ROOT / "runs" / "direction_a_v6" / "reports" / "composite_cells.csv"
# all paper figures live in figures/paper/ (single place for plots)
PLOTS_DEFAULT = ROOT / "figures" / "paper"
...
#: axes every composite view needs; a cell missing any of them cannot be plotted.
_REQUIRED = ("P", "Q", "S", "raw_hac", "sfs")


def load_cells(path: Path) -> list[dict]:
    """Load the composite cell table.

    Cells whose axes are undefined -- no coherence-gate-passing response, or no
    paired trace/answer -- carry empty fields. They are dropped here rather than
    coerced to 0.0, which would understate the axes instead of abstaining.
    """
    cells, dropped = [], []
    with path.open() as f:
        for row in csv.DictReader(f):
            ...
            if any(c[k] is None for k in _REQUIRED):
                dropped.append((c["model"], c["dataset"], c["condition"]))
                continue
            cells.append(c)
    if dropped:
        print(f"[load] {len(dropped)} cells dropped (undefined axes), "
              f"{len(cells)} retained")
    return cells


def pooled_points(cells: list[dict]) -> list[dict]:
    """Dataset-pooled per-(model, condition) means.

    ``sfs`` is the MEAN OF THE PER-CELL SFS values -- the same aggregation every
    paper table uses -- so figures and tables agree. ``sfs_of_pooled_axes`` (SFS
    recomputed from the averaged P, Q, S) is kept only for drawing iso-SFS
    contours in the 2-D Pareto projection, where a point's position is its
    pooled axes."""
    ...
    for (model, cond), rows in grp.items():
        ...
        out.append({
            ...
            "P": p, "Q": q, "S": s, "R": q * s,
            "sfs": float(np.mean([r["sfs"] for r in rows])),
            "sfs_of_pooled_axes": _geo_sfs(p, q, s),
...
def _save(fig, plots_dir: Path, name: str) -> None:
    for path in save_fig(fig, plots_dir / name):
...
def plot_compact_dose_response(pooled: list[dict], plots_dir: Path) -> None:
    ...
    fig, axes = plt.subplots(len(panels), 1, figsize=(5.4, 4.2), sharex=True)
    for ax, (key, title) in zip(axes, panels):
        ...
        ax.set_ylim(-0.02, 1.02)
        ax.grid(alpha=.3, axis="y")
    axes[-1].set_xticks(ticks, ticklabels, fontsize=10)
...
def plot_model_family_heatmap(cells: list[dict], plots_dir: Path) -> None:
    ...
    fam_cols = sorted(FAMILIES)  # stable alphabetical column order
    grid = np.full((len(models), len(fam_cols)), np.nan)
    for i, m in enumerate(models):
        for j, fam in enumerate(fam_cols):
            vals = [c["sfs"] for c in cells
                    if c["model"] == m and c["family"] == fam]
            if vals:
                grid[i, j] = np.mean(vals)
    fig, ax = plt.subplots(figsize=(5.4, 1.0 + 0.72 * len(models)))
    im = ax.imshow(grid, cmap="viridis", vmin=0, aspect="auto")
    for i in range(len(models)):
        for j in range(len(fam_cols)):
            v = grid[i, j]
    ...
    ax.set_xticks(range(len(fam_cols)), fam_cols, rotation=20)
...
def main() -> int:
    ...
    # The paper reports five model/configuration arms; r1_distill is exploratory
    # and is not shown in the paper figures.
    cells = [c for c in cells if c["model"] != "r1_distill_qwen_7b"]
    ...
    for name, html_text in [
        ...
        path = out_dir / name
        # the builders link images as "plots/<name>.png"; point those links at the
        # real figures directory relative to where the report is written
        rel = os.path.relpath(plots_dir, out_dir).replace(os.sep, "/")
        html_text = html_text.replace('src="plots/', f'src="{rel}/')
