...
Self-contained companion to ``scripts/validation/score_annotations.py``: recomputes the
...
Usage:
    python scripts/plotting/plot_validation.py --batch data/annotations/batch_v5_002
"""
...
def main() -> int:
    ...
    A = compute(batch)
    # all plots live under figures/; one folder per annotation batch
    written = render(A, Path(__file__).resolve().parents[2] / "figures" / "validation" / batch.name)
