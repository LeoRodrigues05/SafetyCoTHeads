...
Usage:
    .venv/bin/python -m scripts.unused.v5_reports.make_intro_teaser
"""
...
from _paper_figstyle import apply_style  # noqa: E402
METRICS_DEFAULT = ROOT / "runs" / "direction_a_v6" / "reports" / "cell_metrics.json"
...
CELLS = [
    dict(key=("olmo3_7b_base", "jbb", "neurons_top512"),
         label="OLMo-3 Base\nneurons top-512"),
    dict(key=("llama31_8b_control", "jbb", "steering_a1.5"),
         label="Llama-3.1-8B\nsteering α=1.5"),
    dict(key=("llama31_8b_control", "jbb", "steering_a0.5"),
         label="Llama-3.1-8B\nsteering α=0.5"),
]
...
def load_cells(metrics_path: Path) -> list[dict]:
    """Read the v6 bundle. Cells with no gate-passing response have null axes;
    they plot as 0 so the figure shows the collapse rather than dropping the bar."""
    rows = json.loads(metrics_path.read_text())["rows"]
    by_key = {(r["model"], r["dataset"], r["condition"]): r for r in rows}
    ...
    for spec in CELLS:
        ...
        out.append({**spec,
                    "asr": num(row["hac"]),
                    "P": num(row["P"]), "Q": num(row["Q"]),
                    "S": num(row["S_v6"]), "sfs": num(row["SFS"])})
    return out
...
def build(cells: list[dict], out_stem: Path) -> None:
        ...
        for xi, v in zip(x, vals):
            if xi in (0, len(METRICS) - 1):
                ax.text(xi + (s - 1) * w, v + 0.015, f"{v:.2f}", ha="center",
                        va="bottom", fontsize=7.0, color=INK_MUTED)
    ...
    ax.text(2.0, 1.02, "decomposition", ha="center", va="bottom",
            fontsize=8.0, color=INK_MUTED, style="italic")

    ax.set_ylim(0, 1.08)
    ax.set_yticks([0, 0.25, 0.5, 0.75, 1.0])
    ax.set_xticks(x)
    ax.set_xticklabels([m for m, _ in METRICS], fontsize=8.5, linespacing=0.95)
    ax.set_ylabel("Score (0–1)", fontsize=9.0)
    ...
    fig.suptitle("High ASR is not a diagnosis", fontsize=12,
                 fontweight="bold", color=INK, y=0.985)
    leg = ax.legend(loc="lower center", bbox_to_anchor=(0.5, 1.005), ncol=3,
                    fontsize=7.0, handlelength=0.9, handleheight=1.4,
...
def main() -> int:
    cells = load_cells(METRICS_DEFAULT)
