...
#
# Usage:
#   bash scripts/unused/v5_pipeline/run_3gpu_all.sh                # run the whole DAG on GPUs 0,1,2
#   NGPU=2 bash scripts/unused/v5_pipeline/run_3gpu_all.sh         # use fewer cards
#   bash scripts/unused/v5_pipeline/run_3gpu_all.sh --plan         # print the task DAG and exit
# =============================================================================
set -uo pipefail
cd "$(dirname "$0")/../../.."
ROOT="$(pwd)"
...
judge_one() { # <layer:std|pathway> <model> <dset> <gpu>
  ...
  env CUDA_VISIBLE_DEVICES=$gpu "${env[@]}" "$PY" -m scripts.unused.v5_judging.run_v4_jbb_judge \
...
run_task() { # <id> <gpu>
  ...
  case "$kind" in
    disc)
      if [[ "$a" == r1 ]]; then
        CUDA_VISIBLE_DEVICES=$gpu bash scripts/unused/v5_pipeline/run_local_pipeline.sh r1_distill_qwen_7b discover \
          && touch "$STATE/disc_r1.done"
      else
        CUDA_VISIBLE_DEVICES=$gpu "$PY" -m scripts.discovery.run_direction_extraction \
          --config "$CFG/llama31_8b_control/17-direction-extraction.yaml" \
          && touch "$STATE/disc_llama.done"
      fi ;;
    gen)
      local gcfg="$CFG/$(mkey $a)/gen/$b/$c.yaml"
      CUDA_VISIBLE_DEVICES=$gpu "$PY" -m scripts.generation.run_generation --config "$gcfg" ;;
    jstd)  judge_one std     "$a" "$b" "$gpu" ;;
    jpath) judge_one pathway "$a" "$b" "$gpu" ;;
    sr)    CUDA_VISIBLE_DEVICES=$gpu "$PY" -m scripts.unused.v5_judging.run_v5_safety_reasoning \
...
"$PY" -m scripts.unused.v5_judging.reaggregate_v5_summaries
"$PY" -m scripts.unused.v5_reports.make_composite_report \
  --out "$RUN/composite_report.html" \
  --csv-out "$RUN/composite_cells.csv" \
  --json-out "$RUN/composite_cells.json"
"$PY" -m scripts.unused.v5_reports.make_v5_plots || echo "  (plots step failed — reports are intact)"
