...
# GPU pinning is supplied by the caller, e.g. `CUDA_VISIBLE_DEVICES=1 bash scripts/unused/v5_pipeline/resume_llama_gpu1.sh`
PY=".venv/bin/python"
...
if [[ ! -f runs/direction_a/01-ships-discovery-llama31/ships_dataset_ranking.json ]]; then
  "$PY" -m scripts.discovery.run_attribution --config "$CFG_SHIPS" || { echo "SHIPS discovery FAILED"; exit 1; }
...
if [[ ! -f runs/direction_a/16-neuron-discovery-llama31/neuron_ranking.json ]]; then
  "$PY" -m scripts.discovery.run_neuron_discovery --config "$CFG_NEURON" || { echo "neuron discovery FAILED"; exit 1; }
...
"$PY" -m scripts.unused.v5_pipeline.complete_v5_generation llama31_8b_control --overrides batch_size=48
