...
#
# Same logic as scripts/unused/v5_pipeline/sbatch/direction_a_v5_per_model.sbatch, but STAGE-GATED
# so generation and judging can be launched separately (and two models can be
# generated in parallel in two tmux panes).
#
# Usage:
#   bash scripts/unused/v5_pipeline/run_local_pipeline.sh <MODEL_KEY> <STAGE>
...
#       bash scripts/unused/v5_pipeline/run_local_pipeline.sh olmo2_7b_instruct discover
...
do_discover() {
    [[ -f "$CFG_DIR/01-ships-discovery.yaml" ]] && {
        ...
        python -m scripts.discovery.run_attribution --config "$CFG_DIR/01-ships-discovery.yaml"; }
    [[ -f "$CFG_DIR/16-neuron-discovery.yaml" ]] && {
        echo "=== [$MODEL_KEY] neuron discovery ==="
        python -m scripts.discovery.run_neuron_discovery --config "$CFG_DIR/16-neuron-discovery.yaml"; }
    [[ -f "$CFG_DIR/17-direction-extraction.yaml" ]] && {
        echo "=== [$MODEL_KEY] refusal-direction extraction ==="
        python -m scripts.discovery.run_direction_extraction --config "$CFG_DIR/17-direction-extraction.yaml"; }
}
...
do_gen() {
    for dset_dir in "$CFG_DIR/gen"/*/; do
        ...
        for gcfg in "$dset_dir"*.yaml; do
            ...
            python -m scripts.generation.run_generation --config "$gcfg"
        done
    done
}
...
do_judge() {
    for dset_dir in "$CFG_DIR/gen"/*/; do
        ...
        python -m scripts.unused.v5_judging.run_v4_jbb_judge \
        ...
        python -m scripts.unused.v5_reports.make_v4_jbb_report \
