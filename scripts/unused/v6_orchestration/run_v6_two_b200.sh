...
#
# Usage:
#   bash scripts/unused/v6_orchestration/run_v6_two_b200.sh <stage> [options]
# Stages:
...
# proven scripts/unused/v5_judging/run_sr_vllm.sh recipe so libcudart.so.13/nvcc resolve and the
...
run_audit() {
  log "STAGE audit"
  $PY scripts/preprocessing/audit_v6_generations.py $(mopt) $(dopt) 2>&1 | tee "$LOGDIR/audit_$STAMP.log"
}

run_parse() {
  log "STAGE parse (CPU)"
  $PY scripts/preprocessing/parse_v6_completions.py $(mopt) $(dopt) 2>&1 | tee "$LOGDIR/parse_$STAMP.log"
}
...
judge_stage() {
  ...
  CUDA_VISIBLE_DEVICES=0 $PY scripts/judging/run_v6_judge_shard.py --stage "$stage" \
      --gpu 0 --n-shards 2 --backend "$BACKEND" $(mopt) $(dopt) "${extra[@]}" \
      >"$LOGDIR/${stage}_gpu0_$STAMP.log" 2>&1 &
  local p0=$!
  CUDA_VISIBLE_DEVICES=1 $PY scripts/judging/run_v6_judge_shard.py --stage "$stage" \
...
run_aggregate() {
  ...
  $PY scripts/analysis/aggregate_v6_metrics.py --n-boot "$N_BOOT" --answer-source "$src" \
      $(mopt) $(dopt) 2>&1 | tee "$LOGDIR/aggregate_$STAMP.log"
  $PY scripts/plotting/plot_v6_figures.py 2>&1 | tee -a "$LOGDIR/aggregate_$STAMP.log" || \
    log "plotting skipped (matplotlib unavailable)"
  $PY scripts/release/stage_v6_hf_export.py 2>&1 | tee -a "$LOGDIR/aggregate_$STAMP.log"
  $PY scripts/release/write_v6_manifest.py 2>&1 | tee -a "$LOGDIR/aggregate_$STAMP.log"
}

run_validation() {
  log "STAGE validation (CPU, reuse annotations)"
  $PY scripts/validation/reproduce_v5_validation.py 2>&1 | tee "$LOGDIR/validation_$STAMP.log"
  $PY scripts/training/eval_pathway_judge.py --help >/dev/null 2>&1 && \
    log "pathway transfer-domain validation available via scripts/training/eval_pathway_judge.py (HarmThoughts held-out)" || true
}

run_check() {
  log "STAGE check"
  $PY scripts/preprocessing/check_v6_completeness.py $(mopt) $(dopt) 2>&1 | tee "$LOGDIR/check_$STAMP.log"
}

run_smoke() {
  log "STAGE smoke (2 cells, dry-run judging: builds sharded inputs, no model)"
  local sm_models="olmo3_7b_think" sm_ds="jbb"
  $PY scripts/preprocessing/audit_v6_generations.py --models $sm_models --datasets $sm_ds
  $PY scripts/preprocessing/parse_v6_completions.py --models $sm_models --datasets $sm_ds
  for st in answer monitor; do
    CUDA_VISIBLE_DEVICES=0 $PY scripts/judging/run_v6_judge_shard.py --stage $st --gpu 0 --n-shards 2 \
      --models $sm_models --datasets $sm_ds --dry-run
    CUDA_VISIBLE_DEVICES=1 $PY scripts/judging/run_v6_judge_shard.py --stage $st --gpu 1 --n-shards 2 \
      --models $sm_models --datasets $sm_ds --dry-run
  done
  $PY scripts/analysis/aggregate_v6_metrics.py --models $sm_models --datasets $sm_ds --n-boot 200 --answer-source v5
  $PY scripts/preprocessing/check_v6_completeness.py --models $sm_models --datasets $sm_ds
