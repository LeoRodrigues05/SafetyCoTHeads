...
#
# Usage:
#   bash scripts/unused/v6_orchestration/v6_bash_compiled_runs.sh                # full pipeline
#   bash scripts/unused/v6_orchestration/v6_bash_compiled_runs.sh --from monitor # resume from a stage
#   BACKEND=vllm bash scripts/unused/v6_orchestration/v6_bash_compiled_runs.sh   # use vLLM instead of HF
#   N_BOOT=10000 bash scripts/unused/v6_orchestration/v6_bash_compiled_runs.sh
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
# vLLM env (only if BACKEND=vllm) — mirrors scripts/unused/v5_judging/run_sr_vllm.sh.
...
judge_dual_gpu() {
  ...
  local rc=0
  $PY scripts/judging/run_v6_dual_gpu.py --stage "$stage" --gpus 2 --backend "$BACKEND" \
...
# ---- CPU preflight ---------------------------------------------------------
if ! skip audit; then log "STAGE audit"; $PY scripts/preprocessing/audit_v6_generations.py >>"$MASTER_LOG" 2>&1; fi
if ! skip parse; then log "STAGE parse"; $PY scripts/preprocessing/parse_v6_completions.py >>"$MASTER_LOG" 2>&1; fi
if ! skip check; then log "STAGE check"; $PY scripts/preprocessing/check_v6_completeness.py >>"$MASTER_LOG" 2>&1 || log "  check reported issues (see log)"; fi

# ---- GPU judging (both B200s) ---------------------------------------------
if ! skip answer; then
  judge_dual_gpu answer  "$BATCH_ANSWER"  "$TOK_ANSWER"
  log "STAGE coherence (CPU, answer_text)"; $PY scripts/judging/run_v6_judge_shard.py \
...
if ! skip aggregate; then
  ...
  $PY scripts/analysis/aggregate_v6_metrics.py --n-boot "$N_BOOT" --answer-source "$src" >>"$MASTER_LOG" 2>&1
  $PY scripts/plotting/plot_v6_figures.py >>"$MASTER_LOG" 2>&1 || log "  plotting skipped"
  $PY scripts/release/stage_v6_hf_export.py >>"$MASTER_LOG" 2>&1
  $PY scripts/release/write_v6_manifest.py >>"$MASTER_LOG" 2>&1
fi
if ! skip validation; then
  log "STAGE validation (reuse annotations)"
  $PY scripts/validation/reproduce_v5_validation.py >>"$MASTER_LOG" 2>&1
fi
