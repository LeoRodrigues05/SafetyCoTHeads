...
# scripts/judging/run_v6_dual_gpu.py with the fixed build_inputs (cumulative-prefix
...
#
# Usage (in tmux):
#   tmux new -s f2 'bash scripts/unused/v6_orchestration/rerun_finding2_p04.sh 2>&1 | tee -a runs/direction_a_v6/logs/finding2_p04.out'
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
$PY scripts/judging/run_v6_dual_gpu.py --stage pathway --gpus 2 --backend "$BACKEND" \
...
$PY scripts/judging/run_v6_dual_gpu.py --stage safety-reasoning --gpus 2 --backend "$BACKEND" \
...
$PY scripts/analysis/aggregate_v6_reasoning.py --models $MODELS --datasets $DATASETS \
