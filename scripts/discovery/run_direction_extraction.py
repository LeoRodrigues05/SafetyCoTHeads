"""Extract the refusal direction (Arditi et al. 2024).

Usage:
    python -m scripts.discovery.run_direction_extraction \
        --config configs/experiments/direction_a_ships/17-direction-extraction-llama31.yaml
"""
...
def main() -> int:
    ...
    ext_cfg = DirectionExtractionConfig(
    ...
    npz_path = out_dir / "refusal_directions.npz"
    save_directions_npz(npz_path, result)
    meta = {k: v for k, v in result.items() if k not in {"directions", "mean_harmful", "mean_benign"}}
    meta["npz_path"] = str(npz_path)
    meta["n_layers"] = int(result["directions"].shape[0])
    meta["hidden_size"] = int(result["directions"].shape[1])
    json_dump(out_dir / "refusal_directions_meta.json", meta)
    log.info("wrote %s (default_layer=%d)", npz_path, meta["default_layer"])
    return 0
