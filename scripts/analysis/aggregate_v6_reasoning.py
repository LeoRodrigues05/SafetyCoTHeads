...
def _pathway_cell(cell: C.Cell) -> dict | None:
    ...
    # provenance: which judge + protocol produced these labels. Every v6 cell
    # before 2026-09-24 was the zero-shot 30B on the multi-label prompt, not the
    # validated 14B single-label judge.
    judges = sorted({str(r.get("judge_model")) for r in rows})
    protocol = ("14b_single_label" if all(r.get("judge_kind_source") == "single_label_merge"
                                          for r in rows) else "multi_label")
    return {"n_prefix_rows": len(rows), "n_completions": len(vectors), "summary": summary,
            "judge_models": judges, "protocol": protocol}
...
def main():
    ...
    ap.add_argument("--conditions", nargs="*", default=None,
                    help="restrict to these conditions (the cells whose pathway labels were "
                         "produced under the required protocol)")
    ap.add_argument("--require-pathway-protocol", default="14b_single_label",
                    help="refuse to write a report pooling cells judged under another "
                         "pathway protocol ('any' to disable, for debugging)")
    args = ap.parse_args()
    ...
    for c in cells:
        ...
        out_rows.append({
            ...
            "pathway_judge_models": (pw or {}).get("judge_models"),
            "pathway_protocol": (pw or {}).get("protocol"),
        })

    bad = [f"{r['model']}/{r['dataset']}/{r['condition']}={r['pathway_protocol']}"
           for r in out_rows if r["pathway_protocol"] not in (None, args.require_pathway_protocol)]
    if bad and args.require_pathway_protocol != "any":
        raise SystemExit(f"[reasoning] {len(bad)} cells have pathway labels from another "
                         f"protocol (e.g. {bad[:3]}); run `run_v6_dual_gpu.py --stage "
                         f"pathway14b` first, or pass --require-pathway-protocol any")
    ...
    cols = ["model", "dataset", "condition", "is_primary",
            ...
            "sr_mean_sentence_count", "sr_mean_first_position_norm", "pathway_protocol"]
