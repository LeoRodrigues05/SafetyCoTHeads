...
Usage:
    ...
    python scripts/training/eval_pathway_judge.py \
...
def _run_judge_on_examples_vllm(examples: list[dict], model_path: str) -> list[bool]:
    """Same evaluation on the vLLM backend (continuous batching, greedy, 96 tokens).

    HF and vLLM greedy outputs are not identical, so the judge's reliability has
    to be re-measured under whichever backend produces the paper's labels.
    """
    ...
    judge = load_vllm_judge(model_path, dtype="bfloat16", max_model_len=16384,
                            gpu_memory_utilization=0.90, tensor_parallel_size=1, seed=0)
    raws = judge.generate([ex["user_prompt"] for ex in examples], 96, 0.0,
                          use_chat_template=True)
    out = []
    for raw in raws:
        parsed = parse_judge_json(raw)
        data = parsed.get("data") if parsed.get("parse_status") in ("ok", "recovered") else None
        out.append(bool((normalize_single_label(data) or {}).get("label_present", False)))
    return out
...
def _run_judge_on_examples(
    ...
    *,
    ...
    if backend == "vllm":
        return _run_judge_on_examples_vllm(examples, model_path)
    import torch
...
def main() -> int:
    ...
    ap.add_argument("--backend", choices=["hf", "vllm"], default="hf",
                    help="evaluate under the backend that will produce the paper's labels")
    ...
    ft_preds = _run_judge_on_examples(
        examples, args.finetuned_model, batch_size=args.batch_size, backend=args.backend)
    ...
    results: dict = {"per_label": {}, "backend": args.backend,
                     "finetuned_model": args.finetuned_model, "n_examples": len(examples)}
