...
from safety_cot_heads.direction_a_v6.parsing import (
    parse_completion, prompt_prefills_trace, PARSER_VERSION)

#: parsed fields that feed a judge; a change in any of them invalidates that
#: row's downstream judgments (see scripts/preprocessing/invalidate_v6_rows.py).
JUDGED_FIELDS = ("answer_text", "trace_text", "trace_kind", "prose_prefix_text")
...
def process_cell(cell: C.Cell) -> dict:
    ...
    # Previous parse of this cell, for the change report below.
    old = {str(r.get("id")): r for r in C.read_jsonl(out)} if out.exists() else {}
    parsed_rows = []
    kinds = Counter()
    status = Counter()
    ...
    n_trace_empty = 0
    for r in rows:
        prefilled = prompt_prefills_trace(r.get("rendered_prompt"))
        p = parse_completion(r.get("completion"), trace_prefilled=prefilled)
        ...
        parsed_rows.append({
            ...
            "trace_prefilled": prefilled,
            "parser_version": PARSER_VERSION,
            # generation provenance: continued rows come from gen_continued/
            "gen_continued": bool(r.get("continued")),
    ...
    # Change report: which rows' judged inputs differ from the previous parse.
    # Judge outputs are keyed by id only, so without this a re-parse would leave
    # stale labels in place (resume skips ids that already exist).
    changed = {}
    for pr in parsed_rows:
        prev = old.get(str(pr["id"]))
        if prev is None:
            continue
        diff = [f for f in JUDGED_FIELDS if _sha(prev.get(f)) != _sha(pr.get(f))]
        if diff:
            changed[str(pr["id"])] = diff
    if old:
        C.write_json(cell.v6_parsed_dir() / "parse_changes.json", {
            "generated_at_utc": C.utcnow_iso(),
            "parser_version_old": next(iter(old.values())).get("parser_version"),
            "parser_version_new": PARSER_VERSION,
    ...
    return {
        ...
        "n_changed_vs_previous_parse": len(changed),
    }
...
def main():
    ...
    n_chg = sum(d["n_changed_vs_previous_parse"] for d in diags)
    print(f"[parse] rows whose judged inputs changed vs previous parse: {n_chg} "
          f"(per-cell parse_changes.json; run scripts/preprocessing/invalidate_v6_rows.py next)")
